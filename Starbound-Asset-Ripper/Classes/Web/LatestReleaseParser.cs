...
namespace Web.Utils
{
    public static class LatestReleaseParser
    {
        ...
        {
            Dictionary<string, object> parsedJson = WebUtils.ParseJson(JsonURL, TimeOut);
        ...
        public static string GetLatestRelease(string GithubUserName, string GithubRepoName, int TimeOut)
        {
            ...
            if (WebUtils.CanConnect(fullURL, TimeOut))
            {
                if (RepoFound(fullURL, TimeOut))
                {
                    Dictionary<string, object> parsedJson = WebUtils.ParseJson(fullURL, TimeOut);
